...
import counters.minter_sdk.Minter.*
import org.json.JSONObject
import counters.minter_sdk.Minter.CoinObjClass.CoinObj
import counters.minter_sdk.Minter.Enum.TxPool
import counters.minter_sdk.Minter.Enum.TransactionTypes as NewTransactionTypes
...
//            coin = address
            ...
            fun(jsonObject: JSONObject, type: Int) {
                if (type == NewTransactionTypes.BUY_SWAP_POOL.int || type == NewTransactionTypes.SELL_SWAP_POOL.int || type == NewTransactionTypes.SELL_ALL_SWAP_POOL.int) {
...
//            println("TransactionTypes $type $data")
                    ...
                    if (type == NewTransactionTypes.TypeSend.int ) {
...
//                        to = getToWallet(data.getString("new_owner"))
                    ...
                    else if (type == NewTransactionTypes.BUY_SWAP_POOL.int) {
...
//                        coin2 = this.getCoin(coin2.id, coin2.symbol, getCoin2)
                    ...
                    else if (type == NewTransactionTypes.SELL_SWAP_POOL.int) {
                        val coin_to_sell = tags!!.getLong("tx.coin_to_sell")
                        val coin_to_buy = tags.getLong("tx.coin_to_buy")
...
//                        coin2 = this.getCoin(coin2.id, coin2.symbol, getCoin2)
                        ...
                        optDouble = minterMatch.getAmount(tags!!.getString("tx.return"))
                        optString = data.getString("minimum_value_to_buy")
                    } else if (type == NewTransactionTypes.SELL_ALL_SWAP_POOL.int) {
                        val coin_to_sell = tags!!.getLong("tx.coin_to_sell")
                        val coin_to_buy = tags.getLong("tx.coin_to_buy")
                        coin = coinObjMap[coin_to_sell]!!
                        coin2 = coinObjMap[coin_to_buy]!!
